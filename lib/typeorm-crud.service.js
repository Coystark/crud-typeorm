...
class TypeOrmCrudService extends crud_1.CrudService {
    ...
    async createMany(req, dto) {
        ...
        const bulk = dto.bulk.map((one) => this.prepareEntityBeforeSave(one, req.parsed)).filter((d) => !(0, util_1.isUndefined)(d));
    ...
    async deleteOne(req) {
        ...
        const toReturn = returnDeleted ? (0, class_transformer_1.plainToClass)(this.entityType, { ...found }) : undefined;
    ...
    prepareEntityBeforeSave(dto, parsed) {
        ...
        return dto instanceof this.entityType
            ...
            : (0, class_transformer_1.plainToClass)(this.entityType, { ...dto, ...parsed.authPersist });
    }
    ...
    builderSetWhere(builder, condition, field, value, operator = '$eq') {
        ...
        const args = [{ field, operator: (0, util_1.isNull)(value) ? '$isnull' : operator, value }, index, builder];
    ...
    checkFilterIsArray(cond, withLength) {
        if (!Array.isArray(cond.value) || !cond.value.length || (!(0, util_1.isNil)(withLength) ? withLength : false)) {
